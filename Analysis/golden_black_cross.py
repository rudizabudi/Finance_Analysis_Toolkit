...
def GB_Cross(path, selection):
    ...
    if 'Price_Data' in path:
        ...
        for i in range(0, 3 * len(df.columns.values)):
            if name1 not in df.columns[i] and name2 not in df.columns[i]:
                ...
                if faster_ma_type == 'Simple Moving Average':
                    ...
                    df[str(df.columns[i]) + name1] = df[str(df.columns[i])].ewm(span=faster_ma, adjust=False).mean()

                df.insert(i + 2, df.columns[i] + name2, '')
                if slower_ma == 'Simple Moving Average':
                    df[str(df.columns[i]) + name2] = df[str(df.columns[i])].rolling(slower_ma).mean()
                elif slower_ma_type == 'Exponential Moving Average':
                    df[str(df.columns[i]) + name2] = df[str(df.columns[i])].ewm(span=slower_ma).mean()
            ...
            if name2 in column:
                for i, row in enumerate(df[column].values):
                    if df[column].iloc[i] != 0:
                        if ops[oplt](df[old_column].iloc[i - 1], df[column].iloc[i - 1]) and ops[opgt](df[old_column].iloc[i], df[column].iloc[i]):
                            index_old = df.index[i]
                            basename = column[:column.index('_')]
                            start_price = df[basename].iloc[i]
                        if ops[opgt](df[old_column].iloc[i - 1], df[column].iloc[i - 1]) and ops[oplt](df[old_column].iloc[i], df[column].iloc[i]) and index_old != 0:
                            time = dt.datetime.strptime(df.index[i], '%Y-%m-%d') - dt.datetime.strptime(index_old, '%Y-%m-%d')
                            duration_list.append(int(time.days))
                            end_price = df[basename].iloc[i]
                ...
                if absolute_list != []:
                    ...
                    total_count_list.append(len(absolute_list))
        ...
        results = open(os.getcwd() + '\\Analysis_Results\\' + filename + '.txt', 'w')
        print(total_absolute_list)
        results.write('--> Total ' + analysis + ' Occurrences: ' + str(np.sum(total_count_list)) + '\n')
